...
public class ScanMaster__1 : ClockBattery
{
    ...
    private void ScanStart_()
    {
        if (BoolCheckObjOnScanner())  // 스캔 성공
        {
            ...
            nowCoroutine = StartCoroutine(asdasdasd());
        }
        else
        {
            RotateObject((int)fCurClockBattery + 5);
            nowCoroutine = StartCoroutine(Scan_Fail());
        }
    ...
    private IEnumerator asdasdasd()
    {
        ...
        transform_RightHand.position + new Vector3(0, 15f, 0), 5f);
    ...
    // #. 스캔 성공!
    private IEnumerator Scan_Success()
    {
        InGameUIController.Instance.bIsUIDoing = true;
        GameAssistManager.Instance.PlayerInputLockOn();

        List<ColorObj> colorObjs = new List<ColorObj>(); // 리스트 생성
        //cameraEvent.CameraTriggerStart(3);
